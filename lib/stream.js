...
Stream.prototype._transform = function _transform( chunk, encoding, clbk ) {
	...
	// If we have newline delimited data, concat any previous incomplete line, re-encode (if need be) each line according to its original encoding, push each line to the destination, and set anything leftover as the new line buffer...
	else {
		line = this._buffer + lines[ 0 ];
		line = encode( line, encoding );
		this.push( line, encoding );
		for ( i = 1; i < len; i++ ) {
			line = encode( lines[ i ], encoding );
			this.push( line, encoding );
		}
		this._buffer = lines[ len ];
	}
...
Stream.prototype._flush = function _flush( clbk ) {
	...
	if ( line ) {
		line = encode( line, this._encoding );
